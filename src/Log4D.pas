...
begin
  inherited Init;
  SetOption(DateFormatOpt, {$IFDEF DELPHIXE2_UP}FormatSettings.{$ENDIF}ShortDateFormat);
end;
...
begin
  Result := nil;
  if SameText(Name, 'ANSI') then
    Result := TEncoding.Default
  else if SameText(Name, 'ASCII') then
    Result := TEncoding.ASCII
  else if SameText(Name, 'Unicode') then
    Result := TEncoding.Unicode
  else if SameText(Name, 'Big Endian Unicode') then
    Result := TEncoding.BigEndianUnicode
  else if SameText(Name, 'UTF-7') then
    Result := TEncoding.UTF7
  else if SameText(Name, 'UTF-8') then
